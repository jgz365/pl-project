namespace customer_kiosk
{
    public partial class main_menu : Form
    {
        ...
        {
            InitializeComponent();
            // reduce flicker by enabling double buffering on the form
        ...
        // Run every tick of your timer
        private void timer1_Tick(object sender, EventArgs e)
        {
            UpdateClock();
        }

        // Shared method to update the label
        private void UpdateClock()
        {
            main_menu_gclock_label.Text = DateTime.Now.ToString("dddd, MMMM dd, yyyy 'at' hh:mm tt");
        }
        ...
        private async Task InitializeWebCatalogAsync()
        {
            ...
            webCatalog = new Microsoft.Web.WebView2.WinForms.WebView2
            {
                DefaultBackgroundColor = Color.White,
                Name = "main_menu_webview_catalog"
            };

            // Replace only the old product area (flow panel), keep header/sidebar visible.
            webCatalog.Location = doubleBufferedFlowLayoutPanel1.Location;
            webCatalog.Size = doubleBufferedFlowLayoutPanel1.Size;
            webCatalog.Anchor = doubleBufferedFlowLayoutPanel1.Anchor;
        ...
        private void ApplyWebCatalogRoundedCorners()
        {
            ...
            path.AddArc(webCatalog.Width - radius, 0, radius, radius, 270, 90);
            path.AddArc(webCatalog.Width - radius, webCatalog.Height - radius, radius, radius, 0, 90);
            path.AddArc(0, webCatalog.Height - radius, radius, radius, 90, 90);
            path.CloseFigure();
...
<body>
  ...
  <script>
    ...
    data.forEach(d=>{
      ...
      c.innerHTML=`<div class='img'>Sample Image</div><div class='title'>${d.title}</div><div class='sub'>${d.sub}</div><div class='meta'><div><div class='price'>${d.price}</div></div><div class='stock'><div class='sub'>Stock</div>${d.stock}</div></div><div class='btns'><button class='ghost'>Details</button><button class='dark'>Apply</button></div>`;
      c.querySelector('.ghost').addEventListener('click',()=>showDetails(d));
      c.querySelector('.dark').addEventListener('click',()=>{});
      cards.appendChild(c);
    });
