...
{
    /// <summary>
    /// Catalog HTML for UC_main_menu (kiosk embedded inside POSCashier).
    /// Identical to CatalogHtmlProvider except openApplication() uses
    /// JSON.stringify(payload) so WebView2 postMessage receives a string,
    /// not [object Object].
    /// The original CatalogHtmlProvider is left completely untouched.
    /// </summary>
    internal static class UC_CatalogHtmlProvider
    {
...
<body>
  ...
  <script>
    ...
    // ── THE FIX ───────────────────────────────────────────────────────────────
    // WebView2 postMessage REQUIRES a string argument.
    // Passing a raw JS object results in "[object Object]" arriving on the C#
    // side, which silently fails JSON parsing and never triggers navigation.
    // JSON.stringify(payload) converts the object to a proper JSON string first.
    // ─────────────────────────────────────────────────────────────────────────
